package com.greatideasoft.cwm.data.core.firebase
...
import com.greatideasoft.cwm.data.core.log.logWarn
...
internal fun <TResult : Any> Task<TResult>.asSingle(): Single<TResult> = SingleCreate<TResult> { emitter ->
	addOnSuccessListener {
		logDebug(TAG, "TResult is success")
		if (it != null) {
			emitter.onSuccess(it)
		} else {
			emitter.onError(NullPointerException("Task result is null"))
		}
	}
...
internal fun <T : Any> DocumentReference.getAndDeserializeAsSingle(clazz: Class<T>): Single<T> = SingleCreate<T> { emitter ->
	get()
...
internal fun <T : Any> Query.executeAndDeserializeSingle(clazz: Class<T>): Single<List<T>> = SingleCreate<List<T>> { emitter ->
...
internal fun <T : Any> DocumentReference.setAsCompletable(dataClass: T): Completable = CompletableCreate { emitter ->
	this.set(dataClass!!)
		.addOnSuccessListener {
